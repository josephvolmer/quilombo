...
|---|---|
| **AllAccess** | ⚠️ **Blocked from CI.** Its AWS WAF returns 200 to a residential IP but blocks GitHub Actions' datacenter ranges, so it yields ~117 events locally and **0 in the daily run**. Same class of block as Passline, milder. Runs on **Boletia/Boletius**. `/venue/<slug>` pages list events in HTML: **The Roxy Live alone = 47 events** with dates ("14 de Noviembre 2026"). No JSON-LD, but titles+dates are parseable. Known venue slugs: `the-roxy-bar-live`, `the-roxy-bar-and-grill`, `teatro-vorterix`. **Covers The Roxy + Vorterix from your list.** Site is behind **AWS WAF** (`awsWafCookieDomainList`) — throttle. |
